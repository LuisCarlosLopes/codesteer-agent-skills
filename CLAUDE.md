...
Não há vitest.config — os `*.spec.ts` ficam colocados ao lado do fonte (`tools/`, `packages/skills-catalog/src/`).

Há um único projeto Nx (`skills-catalog`), com os targets `test` / `scan` / `compile` envelopando os mesmos scripts: `pnpm exec nx run skills-catalog:compile`.

Gotcha do typecheck: `pnpm test` roda `tsc` sobre o `include` do `tsconfig.base.json`, que hoje cobre apenas `packages/skills-catalog/src/**/*.ts` e `tools/**/*.ts`. Código TS fora desses dois caminhos passa despercebido pelo gate — ao criar um pacote novo, adicione-o ao `include`.

## Arquitetura

O produto é uma **plataforma de catálogo de skills para agentes de IA**. O blueprint completo está em `arquitetura_plataforma_skills_mcp.md`; esta fatia implementa só autoria + gates + compilação de registry. Não há API de upload, publicação NPM, `libs/core`, CLI nem servidor MCP ainda — "subir uma skill" é abrir um PR que adiciona a pasta canônica.
...
`system/shared/sys-001..sys-005` mapeiam os componentes do blueprint (catálogo, `libs/core`, CLI, servidor MCP, portal web); só `sys-001` tem código correspondente hoje — os demais descrevem o alvo. Mudança no pipeline implica atualizar `sys-001` e, se for decisão nova, um `dec-` novo.
